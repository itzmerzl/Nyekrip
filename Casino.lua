...
function CUSTOM_DROP(F) tonumber(F) 
RESULT = GET_DATA(F) WORLD_LOCK = F % 100 DIAMOND_LOCK = math.floor((F % 10000) / 100) BLUE_GEM_LOCK = math.floor((F % 1000000) / 10000) BLACK_GEM_LOCK = math.floor(F / 1000000)

-- // Debugging CONSOLE("Result : "..WORLD_LOCK.. " - " .. DIAMOND_LOCK .. " - " .. BLUE_GEM_LOCK .. " - " ..BLACK_GEM_LOCK)

RunThread(function() while CDROP do if SHOW_COLLECTED then break end if not CDROP then break 
if COOLDOWN then CONSOLE("Please wait a seconds...") Sleep(2000) break end
...
end CHAT(V.SYSTEM.CREDIT.."Dropping "..RESULT) if CDROP then CDROP = false COOLDOWN = true end end end end)
end
...
if COMMAND == "/cd" and VALUE[2] then HAVE_AMOUNT = GET_AMOUNT(242)+(GET_AMOUNT(1796)*100)+(GET_AMOUNT(7188)*10000)+(GET_AMOUNT(11550)*1000000) if tonumber(VALUE[2]) == nil then CONSOLE(D.ALERT.AMOUNT_NIL) return true elseif tonumber(VALUE[2]) == 0 then CONSOLE(D.ALERT.AMOUNT_NOL) return true elseif tonumber(VALUE[2]) > math.floor(HAVE_AMOUNT) then CONSOLE(D.ALERT.AMOUNT_LESS) return true else local DROP_AMOUNT = tonumber(VALUE[2]) if not CDROP then CDROP = true else CDROP = false end CUSTOM_DROP(DROP_AMOUNT) end return true elseif COMMAND == "/cd" and not VALUE[2] then CONSOLE("Make sure provide amount to drop by typing like this : /cd <amount>") return true end
...
if L[0] == "OnConsoleMessage" and not (L[1]:find("spun the wheel") or L[1]:find("FRIEND ALERT:") or L[1]:find("UPDATE RECOMMENDED") or L[1]:find("`#** `$The Gods`` `ohave") or L[1]:find("Collected  ")) then CONSOLE(L[1]) return true end
